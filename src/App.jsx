...
import PhoneInput from './components/PhoneInput.jsx'  // ✅ importas viršuje

// GH Pages atveju įrašyk VITE_API_BASE per GitHub Actions Variables
const API_BASE = import.meta.env.VITE_API_BASE || ''  // ✅ naudosime fetch'e
...
const PRODUCT = {
  ...
  description:
    'Kompaktiškas aliuminio lydinio kubas, kurį galima sukti ir lankstyti viena ranka. Tylus mechanizmas, maloni tekstūra, tinka tiek vaikams (7+) tiek suaugusiems.',
  priceCents: 1299,
  oldPriceCents: 1999,
  images: [
...
export default function App(){
  ...
  const [nameVal, setNameVal] = useState('') // ✅ valdomas vardas

  const subtotal = useMemo(() => qty * PRODUCT.priceCents, [qty])
  const shippingCents = useMemo(
    () => (subtotal >= SHIPPING.freeFromCents || subtotal === 0 ? 0 : SHIPPING.defaultCents),
    [subtotal]
  )
  ...
  async function pay(e){
    ...
    const phone = e.target.phone?.value?.trim() || '' // PhoneInput paslepia pilną +XXX...
    const address = e.target.address.value.trim()
    const city = e.target.city.value.trim()
    const zip = e.target.zip.value.trim()

    if (!nameVal.trim() || !email || !address || !city || !zip || !accepted) return

    try {
      setLoading(true)
      const orderId = nextOrderId()
      const res = await fetch(`${API_BASE}/api/paysera/checkout`, {
        method: 'POST',
        ...
        body: JSON.stringify({
          ...
          orderId,
          locale: 'lt',
          payer: { name: nameVal.trim(), phone, address, city, zip },
          items: [
            { id: PRODUCT.id, title: PRODUCT.title, priceCents: PRODUCT.priceCents, qty, color }
          ]
        })
      })
    ...
    <div>
        ...
        <section className="card p24">
            ...
            <div className="chips" style={{ marginTop: 8 }}>
              {PRODUCT.colors.map((c) => (
                <button
                  ...
                  className={'chip' + (color === c.code ? ' active' : '')}
                >
                  {c.name}
                </button>
              ))}
          ...
          <div style={{ marginTop: 12 }}>
            ...
            <div className="qty" style={{ marginTop: 8 }}>
              <button className="chip" onClick={() => setQty((q) => Math.max(1, q - 1))}>
                −
              </button>
              <input
                className="input"
                ...
                onChange={(e) => {
                  ...
                  setQty(Math.max(1, isNaN(n) ? 1 : n)) // ✅ saugiau
                }}
              />
              <button className="chip" onClick={() => setQty((q) => q + 1)}>
                +
              </button>
            </div>
          </div>
          ...
          <div className="row" style={{ marginTop: 12 }}>
            <div>
              <strong>Iš viso: {eur(total)}</strong>
            </div>
            <div className="mini">
              {shippingCents === 0 ? 'Nemokamas pristatymas' : `Pristatymas ${eur(shippingCents)}`}{' '}
          ...
          <form onSubmit={pay} className="grid2" noValidate={false}>
            <div>
              ...
              <input
                ...
                required // ✅ privalomas
              />
              {submitted && !nameVal.trim() && <div className="warn">Privalomas laukas</div>}
            </div>
            ...
            <div>
              ...
              <input
                ...
                autoComplete="email"
                onInvalid={(e) =>
                  e.target.setCustomValidity('Įveskite galiojantį el. pašto adresą')
                }
            ...
            <div>
              ...
              <input
                className="input"
                name="address"
                placeholder="Gatvė, namo/buto nr."
                required // ✅ privalomas
              />
            </div>
            <div>
              <div className="mini">Miestas</div>
              <input className="input" name="city" placeholder="Vilnius" required />
            </div>
            <div>
              <div className="mini">Pašto kodas</div>
              <input className="input" name="zip" placeholder="LT-XXXXX" required />
            </div>
            ...
            <div className="row" style={{ gridColumn: '1 / -1' }}>
              <label className="right">
                ...
                />{' '}
                <span className="mini">Sutinku su pirkimo taisyklėmis</span>
              </label>
              {submitted && !accepted && <span className="warn">Reikia sutikti</span>}
              <button className="btn" disabled={loading}>
                {loading ? 'Jungiama…' : 'Tęsti su Paysera'}
              </button>
            </div>
          </form>
      ...
      <div className="wrap" style={{ marginTop: 8 }}>
        <div className="card p16">
          <div className="row">
            <div className="mini">
              © {new Date().getFullYear()} Drivers development, VšĮ | Į.k. 306766699 | PVM
              LT100017094418
            </div>
...
function nextOrderId(){
  ...
  return `CUBE-${y}${m}${d}-001` // demo seka; produkcijoj tvarko serveris
}
